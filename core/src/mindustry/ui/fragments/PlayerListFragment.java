...
public class PlayerListFragment extends Fragment{
    ...
    public void rebuild(){
        ...
        for(var user : players){
            ...
            button.button(Core.input.shift() ? String.valueOf(user.id) : "[#" + user.color().toString().toUpperCase() + "]" + user.name() + (Core.settings.getBool("showuserid") ? " [accent](#" + user.id +")" : ""), Styles.nonetdef, () -> Core.app.setClipboardText(Core.input.shift() ? String.valueOf(user.id) : user.name)).wrap().width(300f).growY().pad(10);
            button.add().grow();
